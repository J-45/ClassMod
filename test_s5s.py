...
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as socket_handle:
    ...
    NAUTH = b'\x00' # Number of authentication methods supported, uint8
    ...
    AssertThat(server_response).IsEqualTo(VER + b'\x00')
    ...
    print(f"Client sent:\tGET / HTTP/1.1 Host: j45.eu")
    # send some data 
    socket_handle.send(b'GET / HTTP/1.1 Host: j45.eu')
    server_response = socket_handle.recv(1024)
    print(f"Client got:\t{server_response}")
